...
struct StudentViewModel {
    ...
    public func sendReqStudent(reqObj: reqDataModel,then completion: @escaping (Result<resDataModel,ServiceError>) -> ()){
        
        let headers: HTTPHeaders = ["Authorization" : AppUtils.getUsrAuthToken()!,
                                    "Content-Type": "application/json"]
        let request = AF.request(Constants.URL_BASE+Constants.URL_MIDDLEWARE+Constants.URL_ROUTES_STUDENT+Constants.URL_PATH_STUDENT_CREATEREQ, method: .post, parameters: reqObj, encoder: JSONParameterEncoder.default,headers: headers)
        print(Constants.URL_BASE+Constants.URL_MIDDLEWARE+Constants.URL_ROUTES_STUDENT+Constants.URL_PATH_STUDENT_CREATEREQ)
        request.responseDecodable(of: ResponseCreateRequestDataModel.self) { (response) in
            switch response.result {
            case .success(_):
                guard let data = response.value else {return}
                if data.result == "OK" {
                    let userdata:resDataModel = data.data
                    completion(.success(userdata))
                }
                else if data.result == "nOK" {
                    completion(.failure(ServiceError.BackEndError(errorMessage: data.message,data: "fail")))
                }
                else {
                    completion(.failure(ServiceError.Non200StatusCodeError(APIError(message: data.result, status: data.message ))))
                }
            case .failure(let error):
                if let afError = error.asAFError {
                    switch afError {
    ...
    public func deleteReq(reqObj: resquestDeleteReq,then completion: @escaping (Result<resDataModel
                                                                                  ,ServiceError>) -> ()){
        
        let headers: HTTPHeaders = ["Authorization" : AppUtils.getUsrAuthToken()!,
                                    "Content-Type": "application/json"]
        
        let request = AF.request(Constants.URL_BASE+Constants.URL_MIDDLEWARE+Constants.URL_ROUTES_STUDENT+Constants.URL_PATH_STUDENT_DELETEREQ, method: .delete, parameters: reqObj, encoder: JSONParameterEncoder.default , headers: headers)
        request.responseDecodable(of: responseDeleteReq.self) { (response) in
            switch response.result {
            case .success(_):
                guard let data = response.value else {return}
                if data.result == "OK" {
