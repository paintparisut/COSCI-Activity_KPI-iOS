...
struct StudentViewModel {
    ...
    public func sendReqStudent(reqObj: reqDataModel,then completion: @escaping (Result<resDataModel,ServiceError>) -> ()){
        ...
        let headers: HTTPHeaders = ["Authorization" : AppUtils.getUsrAuthToken()!,
                                    "Content-Type": "application/json"]
        let request = AF.request(Constants.URL_BASE+Constants.URL_MIDDLEWARE+Constants.URL_ROUTES_STUDENT+Constants.URL_PATH_STUDENT_CREATEREQ, method: .post, parameters: reqObj, encoder: JSONParameterEncoder.default)
