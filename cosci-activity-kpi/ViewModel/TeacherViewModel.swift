...
struct TeacherViewModel {
    ...
    public func sendReqTeacher(reqObj: requestCreateReqTeacherDataModel,then completion: @escaping (Result<resCreateReqTeacherDataModel,ServiceError>) -> ()){
        
        let headers: HTTPHeaders = ["Authorization" : AppUtils.getUsrAuthToken()!,
                                    "Content-Type": "application/json"]
        let request = AF.request(Constants.URL_BASE+Constants.URL_MIDDLEWARE+Constants.URL_ROUTES_TEACHER+Constants.URL_PATH_TEACHER_CREATEREQ, method: .post, parameters: reqObj, encoder: JSONParameterEncoder.default , headers: headers)
        
        request.responseDecodable(of: ResponseRequestTeacherModel.self) { (response) in
            switch response.result {
            case .success(_):
                guard let data = response.value else {return}
                
                if data.result == "OK" {
                    let userdata:resCreateReqTeacherDataModel = data.data
                    completion(.success(userdata))
                }
                else if data.result == "nOK" {
